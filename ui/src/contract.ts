...
export async function connectContract(addr: string) {
  ...
  const auth = new AuthProvider(`${appAddress}`, {
    ...
    chainConfig: {
      ...
      rpcUrl:
        "https://side-divine-patina.ethereum-goerli.discover.quiknode.pro/da5d7e1f010d66a6c052c2aac2bb66b14516754d/", //defaults to 'https://rpc.ankr.com/eth'
    },
  });

  await auth.connect();

  let provider = new ethers.providers.Web3Provider(auth.provider);
  ...
  const walletAddress = await MyWalletDeployer.getDeploymentAddress(
    ENTRYPOINT_ADDR,
    ownerAddress,
    0
  );
...
export async function setRootAndVerifier(
  smartWalletAPI: MyWalletApi,
  aaProvier: ERC4337EthersProvider
) {
  const aaSigner = aaProvier.getSigner();

  const scw = new ethers.ContractFactory(
    MyWallet__factory.abi,
    MyWallet__factory.bytecode
  );
